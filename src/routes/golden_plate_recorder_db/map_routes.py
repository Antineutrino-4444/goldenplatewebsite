...
def _verify_map_email_code(email, code):
    ...
    if (verification.code or '').strip() != normalized_code:
        ...
        if remaining > 0:
            return {'valid': False, 'error': f'Invalid verification code. {remaining} attempts remaining.'}
        return {'valid': False, 'error': 'Too many failed attempts. Please request a new code.'}
...
def send_map_verification_code():
    ...
    if not _is_sac_email(email):
        return jsonify({'error': 'Only @sac.on.ca email addresses can submit to the map'}), 403
...
def verify_map_email_code_endpoint():
    ...
    if not code.isdigit():
        return jsonify({'error': 'Verification code must contain only digits'}), 400

    if len(code) != 6:
        return jsonify({'error': f'Verification code must be exactly 6 digits (received {len(code)})'}), 400
...
def create_map_submission():
    ...
    if len(text_content) > 5000:
        return jsonify({'error': 'Submission text must be 5000 characters or fewer'}), 400
    ...
    if auth_method == 'password':
        ...
        if not password_verified:
            return jsonify({'error': 'Invalid map submission password'}), 403
    else:
        try:
            ...
            map_db_session.rollback()
            return jsonify({'error': 'Failed to validate email verification'}), 500

        if not email_verified:
            return jsonify({'error': 'Please verify your SAC email address before submitting'}), 400

    if shortcut_password:
        if len(shortcut_password) < 6:
            return jsonify({'error': 'Shortcut password must be at least 6 characters long'}), 400
    ...
    if image_file and image_file.filename:
        ...
        if image_mime not in ALLOWED_IMAGE_MIMES:
            return jsonify({'error': 'Image must be a JPG, PNG, WebP, or GIF file'}), 400
    ...
    try:
        ...
        map_db_session.rollback()
        return jsonify({'error': 'Could not submit map entry'}), 500
...
def approve_map_submission(submission_id):
    ...
    if submission.status != 'pending':
        return jsonify({'error': 'Submission is not pending'}), 400
